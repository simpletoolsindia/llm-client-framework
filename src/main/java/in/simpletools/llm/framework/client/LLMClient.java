...
public class LLMClient {
    ...
    public LLMClient(ProviderAdapter adapter, ClientConfig config) { this(adapter, config, new ConversationHistory()); }

    public LLMClient(ProviderAdapter adapter, ClientConfig config, ConversationHistory history) {
        this.adapter = adapter; this.config = config; this.history = history; this.toolRegistry = new ToolRegistry();
    }
    ...
    public String chat(String message, Map<String, String> options) {
        ...
        if (response.hasToolCalls()) return handleToolCalls(response.getToolCalls());
    ...
    public String chat(Message message) {
        history.add(message);
        LLMRequest request = LLMRequest.builder().model(config.getModel()).messages(history.getMessages())
            .tools(tools.isEmpty() ? null : tools).build();
        String reply = adapter.chat(request).getContentOrEmpty();
        history.addAssistant(reply);
        return reply;
    }
    ...
    public void streamChat(String message, Consumer<String> onChunk) {
        history.addUser(message);
        LLMRequest request = LLMRequest.builder().model(config.getModel()).messages(history.getMessages()).stream(true).build();
        adapter.streamChat(request, onChunk);
    }

    public LLMClient registerTool(Object service) {
        toolRegistry.register(service);
        for (ToolRegistry.ToolInfo ti : toolRegistry.getAllTools()) tools.add(toolFromInfo(ti));
        return this;
    }

    public LLMClient registerTool(String name, String description, Function<Map<String, Object>, Object> handler, Map<String, ToolRegistry.ParamInfo> params) {
        toolRegistry.register(name, description, handler, params);
        tools.add(toolFromInfo(toolRegistry.get(name)));
        return this;
    }

    public LLMClient withTools(List<Tool> tools) { this.tools.clear(); this.tools.addAll(tools); return this; }
    ...
    private LLMRequest buildRequest(String message, Map<String, String> options) {
        LLMRequest.Builder builder = LLMRequest.builder().model(config.getModel()).stream(config.isStream())
            .messages(new ArrayList<>(history.getMessages())).addMessage(Message.ofUser(message));
        if (options.containsKey("system")) builder.system(options.get("system"));
        if (options.containsKey("temperature")) builder.temperature(Double.parseDouble(options.get("temperature")));
        if (options.containsKey("tools") || !tools.isEmpty()) builder.tools(tools.isEmpty() ? null : tools);
    ...
    private String handleToolCalls(List<ToolCall> calls) {
        ...
        for (ToolCall call : calls) {
            ...
            try {
                ...
                result = info != null ? info.invoke(args) : "Tool not found: " + toolName;
        ...
        LLMResponse response = adapter.chat(LLMRequest.builder().model(config.getModel()).messages(messages).tools(tools).build());
        String reply = response.getContentOrEmpty();
        history.addAssistant(reply);
        return reply;
    }
    ...
    private Tool toolFromInfo(ToolRegistry.ToolInfo info) {
        ...
        fn.setName(info.getName()); fn.setDescription(info.getDescription());
        fn.setParameters(new HashMap<>());
        info.getParams().forEach((k, v) -> {
            Tool.Function.Param p = new Tool.Function.Param();
            Class<?> t = v.getType();
            p.setType(t == String.class ? "string" : t == int.class || t == Integer.class ? "integer" : "string");
    ...
    public static LLMClient create(ClientConfig config) {
        ProviderAdapter adapter = switch (config.getProvider()) {
